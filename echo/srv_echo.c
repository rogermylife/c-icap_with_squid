...
void *echo_init_request_data(ci_request_t * req)
{
     ...
     */
     ...
     time(&t);
     tmp=localtime(&t);
     if(strftime(echo_data->file_name,sizeof(echo_data->file_name),"%Y_%m_%d_%H_%M_%S",tmp)==0)
         strcpy(echo_data->file_name,"NOOOOOOOWAYYY");
     char rand_str[16];
     sprintf(rand_str,"%d",time(NULL));
     strcat(echo_data->file_name,rand_str);
...
void echo_release_request_data(void *data)
{
    ...
    if(echo_data->body)
    {
        ...
        //sprintf(temp,"mylog/release%ld_%d",time(NULL),rand());
        //sprintf(check,"\n\nbuf %d\nend_buf %d\n read_pos %d\n write_pos %d\nfull %d\n\n",echo_data->body->buf,echo_data->body->end_buf,echo_data->body->read_pos,echo_data->body->write_pos,echo_data->body->full);
        //FILE *data_body = fopen(temp,"wb");
        //char tempXD[]="HAHAHA rogermylife\n";
        //fwrite (echo_data->body->buf , sizeof(char), echo_data->body->end_buf-echo_data->body->buf+1, data_body);
        //sprintf(tempXD,"total %d \n",echo_data->body->end_buf-echo_data->body->buf+1);
        //fwrite(tempXD, sizeof(char),strlen(tempXD),data_body);
        //fwrite(check, sizeof(char),strlen(check),data_body);
        //fclose(data_body);
            ...
            ci_request_t * req)
{
     ...
     if (wbuf && wlen) {
          *wlen = ci_ring_buf_read(echo_data->body, wbuf, *wlen);
          //ci_debug_printf(1, "*** wbuf:%s\n", wbuf);
            char file_path[100];
            //itoa(time(NULL),temp,10);
            sprintf(file_path,"mylog/%s",echo_data->file_name);
            FILE *data_body = fopen(file_path,"ab");
            //char tempXD[]="HAHAHA rogermylife\n";
            fwrite (wbuf , sizeof(char), *wlen, data_body);
            //sprintf(tempXD,"total %d \n",*wlen);
            //fwrite(tempXD, sizeof(char),strlen(tempXD),data_body);
            fclose(data_body);
     }
